...
const {
  ...
  peerAckPlan,
  peerBridgeComponents: modelPeerBridgeComponents,
  peerEdgeTermination: modelPeerEdgeTermination,
  peerGroupBounds,
  peerIsInteractive,
...
} = await import("./model.js?v=20260629-global-fields-v1");
...
function nodeKind(id) {
  if (byId(state.peers, id)) return "peer";
  if (byId(state.sessions, id)) return "session";
  if (byId(state.fragments, id)) return "fragment";
  return "unknown";
}

function nodeDisplayLabel(node, kind = nodeKind(node?.id)) {
  if (!node) return "unselected";
  if (node.label) return node.label;
  if (kind === "peer") return "unlabeled peer";
  if (kind === "session") return "unlabeled session";
  if (kind === "fragment") return "unlabeled PSBT";
...
function signedSatAmount(valueSats) {
  ...
  if (value === 0) return formatSatAmount(0);
...
function psbtFormatLabel(node) {
  if (node.format === "bip174") return "BIP 174";
...
function descriptorSummary(node) {
  ...
  if (!descriptors.length) return "0 known descriptors";
...
function descriptorPayload(id, privacy, descriptor) {
  if (!descriptor.trim() || privacy === "none") return [];
  return [{
    id,
    privacy,
...
function allPaymentRequests() {
  ...
  for (const descriptor of state.descriptors) {
    ...
    if (request) byUri.set(request.uri, request);
  }
...
function updateDescriptorLinkedCoins(record) {
  for (const node of workspacePsbtNodes()) {
    for (const coin of [...node.inputs, ...node.outputs]) {
      if (coin.descriptorId !== record.id) continue;
...
function handlePastedText(text, source = "paste") {
  ...
  if (!value) return 0;
  if (handlePastedCandidate(value, source)) return 1;
  let count = 0;
  for (const line of value.split(/\r?\n/).map((part) => part.trim()).filter(Boolean)) {
    if (handlePastedCandidate(line, source, false)) count += 1;
  }
  if (count) {
    render();
    return count;
...
function parseDemoPsbtExport(value) {
  ...
  if (!compact.startsWith("ptj-demo:")) return null;
  try {
    const parsed = JSON.parse(atob(compact.slice("ptj-demo:".length)));
    if (!["unordered", "bip370", "bip174"].includes(parsed.format)) return null;
    return {
...
function dummyPeerContact(kind) {
  ...
  if (kind === "npub") return `npub1${token.slice(0, 48)}`;
  if (kind === "wormhole") return `7-${token.slice(0, 6)}-${token.slice(6, 12)}`;
...
function contactPeerLabel(kind, value) {
  if (kind === "npub") return `npub ${short(value, 14)}`;
  if (kind === "wormhole") return `wormhole ${short(value, 18)}`;
  if (kind === "relay") return `relay ${short(relayHost(value), 18)}`;
...
function syncPeerViews(session) {
  for (const peerId of session.peers) {
    ...
    if (peer) peer.views[session.id] = mergePayloads(session);
  }
}

function clearSessionConvergence(sessionId) {
  for (const [key, convergence] of Object.entries(state.convergence)) {
    if (convergence.sessionId === sessionId) delete state.convergence[key];
  }
}
...
function convergenceFor(nodeId) {
  if (nodeKind(nodeId) !== "session") return null;
...
function pendingPayloadRowsForNode(nodeId) {
  if (nodeKind(nodeId) !== "session") return new Set();
...
function startConvergence(node, reason, peerIds = node?.peers || [], pendingRows = []) {
  if (!node || nodeKind(node.id) !== "session") return;
...
function acknowledgeConvergence(key, runId, peerId) {
  ...
  if (!convergence || convergence.runId !== runId || !getNode(convergence.sessionId)) return;
...
function abortSession(sessionId, shouldRender = true) {
  ...
  for (const descriptor of state.descriptors) {
    if (descriptor.absorbedInto === session.id) descriptor.absorbedInto = fragment.id;
  }
  state.fragments.push(fragment);
  state.selected = [fragment.id];
...
function absorbFragmentIntoSession(fragmentId, sessionId, shouldRender = true) {
  const fragment = byId(state.fragments, fragmentId);
  const session = byId(state.sessions, sessionId);
  if (!fragment || !session) return null;
...
function visibleSessionsForPeers(...peerIds) {
  return state.sessions.filter((session) =>
    peerIds.some((peerId) => session.peers.includes(peerId) || Boolean(byId(state.peers, peerId)?.views[session.id])),
  );
}
...
function sortPsbtToBip370(nodeId, shouldRender = true) {
  ...
  if (!node || !["fragment", "session"].includes(kind)) return null;
...
function beginWireDraft(event, sourceId) {
  if (event.button !== 0 || !nodeIsSelectable(sourceId)) return;
...
function addJoinWire(sourceId, targetId) {
  if (!hasCompatibleWireAction(sourceId, targetId)) return false;
...
function nearestWireTargetNode(pointer, sourceId) {
  ...
  for (const [id, box] of positions.entries()) {
    if (!wireAttempt(sourceId, id)) continue;
...
function wireAttempt(sourceId, targetId) {
  ...
  if (sourceKind === "peer" && targetKind === "session") {
    if (sessionVisibleToPeerGroup(target, state.peers, peerBridgeGroupContaining(source.id))) return null;
    return { ok: true, label: `Share ${target.label} with ${source.label}` };
  }
  if (sourceKind === "session" && targetKind === "peer") {
    if (sessionVisibleToPeerGroup(source, state.peers, peerBridgeGroupContaining(target.id))) return null;
    return { ok: true, label: `Share ${source.label} with ${target.label}` };
  }
  if (sourceKind === "peer" && targetKind === "peer") {
    if (!peersCanBridge(source.id, target.id)) return null;
...
function connectSelected() {
  ...
  for (const wire of wires) {
    ...
    if (!result) continue;
    applied += 1;
    if (!getNode(leftId) && getNode(result.id)) remap.set(leftId, result.id);
    if (!getNode(rightId) && getNode(result.id)) remap.set(rightId, result.id);
  }
  state.joinWires = [];
  pruneSelection();
...
function remapJoinWiresAfterJoin(consumedWire, result) {
  ...
  for (const wire of state.joinWires) {
    if (joinWireKey(wire.sourceId, wire.targetId) === consumedKey) continue;
    ...
    if (!getNode(sourceId) || !getNode(targetId) || sourceId === targetId || !hasCompatibleWireAction(sourceId, targetId)) continue;
...
function joinComponents(wires = validJoinWires()) {
  ...
  for (const wire of wires) {
    if (!adjacency.has(wire.sourceId)) adjacency.set(wire.sourceId, new Set());
    if (!adjacency.has(wire.targetId)) adjacency.set(wire.targetId, new Set());
...
function positionDescriptorMenu(summary, panel) {
  ...
  const left = Math.min(
    Math.max(8, anchor.right - panelWidth),
...
function descriptorDrawerSources() {
  const sources = [];
  for (const descriptor of state.descriptors) {
    if (!descriptor.hasUtxo) continue;
    sources.push({
      kind: "utxo",
      id: descriptor.id,
...
function renderDescriptorChip(rail, item, options = {}) {
  ...
  `;
  if (!options.unrecognized) appendDescriptorMenu(chip, item);
...
function setHoveredDescriptor(descriptorId) {
  if (state.hoveredDescriptorId === descriptorId) return;
  state.hoveredDescriptorId = descriptorId;
  renderGraph();
}

function coinMatchesHoveredDescriptor(coin) {
  if (!state.hoveredDescriptorId) return false;
  if (state.hoveredDescriptorId === UNRECOGNIZED_DESCRIPTOR_ID) return !coin.descriptorId;
  return coin.descriptorId === state.hoveredDescriptorId;
}

function coinDimmedByHoveredDescriptor(coin) {
  if (!state.hoveredDescriptorId) return false;
  if (state.hoveredDescriptorId === UNRECOGNIZED_DESCRIPTOR_ID) return Boolean(coin.descriptorId);
  return coin.descriptorId !== state.hoveredDescriptorId;
}
...
function renderInspector() {
  ...
  if (kind === "peer") {
    ...
    if (node.sourceDescription) rows.push(["origin", node.sourceDescription]);
  }
  if (kind === "session") {
    rows.push(["sort mode", node.sortMode || "det"]);
    if (node.seed) rows.push(["seed", node.seed]);
...
function renderGraph() {
  ...
  for (const edge of state.edges) {
    if (edge.kind !== "session-peer") continue;
    const peerIds = peerBridgeGroupContaining(edge.to)
      .filter((peerId) => state.edges.some((candidate) =>
        candidate.kind === "session-peer" &&
        candidate.from === edge.from &&
        candidate.to === peerId,
      ));
    const wireKey = convergenceWireKey(edge.from, peerIds);
    if (drawnSessionPeerWires.has(wireKey)) continue;
    ...
    if (convergence) drawEdgeSpinner(svg, from, to, convergence);
  }
...
function requiredGraphWidth(metrics = graphLayoutMetrics()) {
  return Math.max(
    peerBlocksWidth(peerBridgeComponents(remotePeers())),
    sizedRowWidth(state.sessions, "session"),
    sizedRowWidth(state.fragments, "fragment", graphMetrics.fragment.gap),
    metrics.localPeerMinWidth,
  );
}
...
function sizedRowWidth(items, kind, minGap = 36) {
  if (!items.length) return rowWidth(0, graphMetrics[kind].width, minGap);
...
function psbtNodeSize(node, kind) {
  ...
  if (!expandedRows.length) return { width: base.width, height: base.height };
  const detailHeight = expandedRows.reduce((sum, row) =>
    sum + Math.max(0, coinRowHeight(row.side, row.item, row.index) - 34), 0);
  return {
...
function expandedCoinRowsForNode(node) {
  ...
  (node.inputs || []).forEach((item, index) => {
    if (isCoinRowExpanded("input", item)) rows.push({ side: "input", item, index });
  });
  (node.outputs || []).forEach((item, index) => {
    if (isCoinRowExpanded("output", item)) rows.push({ side: "output", item, index });
  });
  return rows;
}
...
function peerGroupWidths(groups) {
  return groups.map((group) =>
    group.length * graphMetrics.peer.width + Math.max(0, group.length - 1) * peerInnerGap(group),
  );
}
...
function wireEndpoint(box, toward) {
  const centerX = box.x + box.width / 2;
  const centerY = box.y + box.height / 2;
  if (!toward) return { x: centerX, y: centerY };
...
function drawWireDraft(svg, positions) {
  ...
  if (target && targetAttempt) drawWireTooltip(svg, target, targetAttempt.label, targetAttempt.ok ? "" : "invalid");
}
...
function drawPeerNode(svg, box, peer) {
  ...
  if (peer.local) group.classList.add("local-peer");
...
function drawPsbtNode(svg, box, node, kind) {
  ...
  if (showPsbtTag) group.appendChild(svgText(12, 22, "PSBT", "psbt-tag"));
  ...
  if (convergence) drawConvergenceBadge(group, box.width - 78, 12, convergence);
...
function drawPsbtColumn(group, node, side, x, y, width, maxY, heading, items, formatter) {
  ...
  for (const [index, item] of items.entries()) {
    ...
    if (cursor + rowHeight > maxY && rendered > 0) break;
...
function drawUnorderedPsbtBalanceSheet(group, node, display, pendingRows, box, padding = 12, startY = 72) {
  ...
  for (const [sectionIndex, section] of display.subtransactions.entries()) {
    ...
    if (!hideHeading && cursor + 14 <= sectionLimitY) {
      ...
      if (section.descriptorColor) label.setAttribute("style", `fill:${section.descriptorColor}`);
    ...
    for (let index = 0; index < rowCount; index += 1) {
      ...
      const rowHeight = Math.max(
        input ? coinRowHeight("input", input, index) : 0,
        output ? coinRowHeight("output", output, index) : 0,
      );
      if (cursor + rowHeight > rowLimitY && renderedRows > 0) break;
      if (input) {
...
function drawCoinRow(group, x, y, width, height, side, coin, index, pending = false) {
  ...
  if (pending) drawConstructionFrame(row, width, height, "coin-construction-marker", true);
  if (coin.descriptorColor) {
...
function inputSignedStatus(side, coin) {
  ...
  if (coin.signatureVerified === true) return "authorized";
...
function coinRowHeight(side, coin, index) {
  if (!isCoinRowExpanded(side, coin)) return 34;
...
function nodeClass(nodeId, kind, selected) {
  ...
  if (state.joinWires.some((wire) => wire.sourceId === nodeId || wire.targetId === nodeId)) classes.push("join-selected");
  if (convergenceFor(nodeId)) classes.push("converging");
  if (state.wireDraft?.sourceId === nodeId) classes.push("wire-source");
  if (state.wireDraft) {
    const attempt = wireAttempt(state.wireDraft.sourceId, nodeId);
    if (attempt?.ok) classes.push("wire-compatible");
    if (attempt && !attempt.ok) classes.push("wire-incompatible");
  }
  if (state.wireDraft?.targetId === nodeId) classes.push("wire-target");
  return classes.join(" ");
}
...
function svgSignedAmountText(x, y, valueSats, className, anchor = "start") {
  ...
  if (value < 0) element.insertBefore(svgTspan("-"), element.firstChild);
  return element;
}
...
function appendAmountPartTspans(element, text, muted) {
  ...
  if (text) element.appendChild(svgTspan(text, muted ? "amount-scale" : null));
}
...
function bindForms() {
  ...
  onSubmit("#descriptorForm", (event) => {
    event.preventDefault();
    addDescriptorUtxo(
      document.querySelector("#descriptorOwner").value,
      document.querySelector("#descriptorText").value,
      document.querySelector("#utxoTxid").value,
      document.querySelector("#utxoVout").value,
      document.querySelector("#utxoValue").value,
      true,
      document.querySelector("#descriptorPrivacy").value,
      document.querySelector("#descriptorOwnership").value,
    );
  });
  ...
  onSubmit("#paymentForm", (event) => {
    event.preventDefault();
    addPaymentIntent(
      document.querySelector("#paymentLabel").value,
      document.querySelector("#paymentAddress").value,
      document.querySelector("#paymentValue").value,
    );
  });
  ...
  generateSessionSeed.addEventListener("click", () => {
    if (!["det", "unset"].includes(sessionOrderingMode.value)) return;
    sessionSeed.value = secureRandomSessionSeed();
  });
