...
const rowDetailOverrides = new Set<string>();

function rowDetailKey(fragmentKey: string, side: "input" | "output", index: number): string {
  return `${fragmentKey}:${side}:${index}`;
}

function rowExpanded(fragmentKey: string, side: "input" | "output", index: number): boolean {
...
function clearRowOverrides(fragmentKey: string): void {
  for (const key of [...rowDetailOverrides]) {
    if (key.startsWith(`${fragmentKey}:`)) rowDetailOverrides.delete(key);
  }
}
...
function settleJoin(operandKeys: readonly string[], resultKey: string): void {
  ...
  for (const operand of operandKeys) {
    if (operand !== resultKey) glideTargets.set(`fragment:${operand}`, resultKey);
  }
}
...
async function settleMint(
  ...
  const keep = el<HTMLInputElement>(keepBoxId).checked;
  if (!keep) settleDerivation(sourceKeys, resultKeys, resultKeys.join(", "));
  render();
  ...
  if (options?.monotone || resultKeys.length !== 1) return;
  ...
  for (const holder of holders) {
      ...
      try {
        ...
        if (joinAdvances(fragmentSummary(inspect), fragmentSummary(result.inspect))) {
          const joined = await addResponse(
            { psbt: response.psbt, inspect },
            "join",
            `⊔ advance of ${holder.name} by ${resultKey}`,
          );
          objects = writeSessionContent(objects, holder.key, joined.key);
          settleJoin([oldContent, resultKey], joined.key);
          logEvent(
            `${holder.name} (${holder.key}) advanced monotonically: ` +
        ...
        logEvent(
          `${holder.name} (${holder.key}): ${oldContent} ⊔ ${resultKey} absorbs the ` +
            `transform back into the old value — not an advance; offering the fork`,
        );
      } catch (error) {
        logEvent(
          `${holder.name} (${holder.key}): ${oldContent} ⊔ ${resultKey} conflicts ` +
    ...
    if (!keep) settleDerivation([oldContent], resultKeys, resultKey);
    render();
  }
}
...
function renderOps(): void {
  ...
  el<HTMLElement>("opsBar").hidden = selected.length === 0;
...
const freshCanvasNodes = new Set<string>();
...
const glideTargets = new Map<string, string>();
...
function renderCanvas(): void {
  ...
  const departing: { wrapper: HTMLElement; resultKey: string }[] = [];
  ...
  for (const { wrapper, resultKey } of departing) {
    ...
    const rect =
      layout.positions.get(`fragment:${resultKey}`) ??
    ...
    placeWrapper(wrapper, rect.x, rect.y);
    const drop = (): void => wrapper.remove();
...
function coinRow(
  ...
  const toggle = () => {
    ...
    if (rowDetailOverrides.has(key)) rowDetailOverrides.delete(key);
    else rowDetailOverrides.add(key);
    render();
  };
  ...
  if (expanded) {
    ...
    const raw = button(
      "Raw",
      `${side} ${index}: every field, raw (address, omitted fields, raw keymap entries)`,
      () => openRawModal(fragment, { side, index }),
    );
...
// the dialog.
function inputRow(input: InputView, expanded: boolean): HTMLElement {
...
function outputRow(output: OutputView, expanded: boolean): HTMLElement {
  const row = document.createElement("div");
  row.className = "session-coin-row";
...
async function addObject(): Promise<void> {
  ...
  const pasted = classifyPaste(raw);
  if (pasted.kind === "psbt") {
    if (await addPsbtText(raw)) settlePaste();
    return;
  }
  if (pasted.kind === "payment-uri") {
    if (await addPaymentUri(pasted.payload)) settlePaste();
    return;
  }
...
function settleSessionFork(fork: boolean): void {
  const dialog = el<HTMLDialogElement>("forkSessionDialog");
  if (dialog.open) dialog.close();
...
function settleSessionDelivery(
  sessionKey: string,
  deliveredPeerKeys: readonly string[],
  fragmentKey: string,
): void {
...
async function syncSessionOverPeer(sessionKey: string, peerKey: string | null): Promise<void> {
  ...
  for (const attempt of broadcastAttempts) {
    if (attempt.startsWith(`${sessionKey}→`)) broadcastAttempts.delete(attempt);
  }
  ...
  if (ok && carrier && content) settleSessionDelivery(sessionKey, sentTo, content.key);
}
...
async function autoBroadcast(
  ...
  if (ok) settleSessionDelivery(sessionObject.key, sentTo, content.key);
}
...
function wireDom(): void {
  ...
  const forkSessionDialog = el<HTMLDialogElement>("forkSessionDialog");
  el<HTMLButtonElement>("forkSessionConfirm").addEventListener("click", () => settleSessionFork(true));
  el<HTMLButtonElement>("forkSessionKeep").addEventListener("click", () => settleSessionFork(false));
  el<HTMLButtonElement>("forkSessionCancel").addEventListener("click", () => settleSessionFork(false));
