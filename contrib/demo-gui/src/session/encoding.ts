...
// ---------------------------------------------------------------------------
// Charset classification: candidate encodings from characters alone. This is
// deliberately NOT validation — a candidate merely deserves a decode attempt.
// ---------------------------------------------------------------------------
...
function bech32Candidate(trimmed: string): boolean {
  // BIP 173 forbids mixed case outright, so it disqualifies candidacy too.
  if (isMixedCase(trimmed)) return false;
  const lowered = trimmed.toLowerCase();
  const separator = lowered.lastIndexOf("1");
  // Nonempty hrp before the separator, at least a checksum's worth after.
  if (separator < 1 || lowered.length - separator - 1 < 6) return false;
  for (const ch of lowered.slice(separator + 1)) {
    if (BECH32_CHARSET.indexOf(ch) === -1) return false;
  }
  return true;
}

export function classifyCharset(text: string): EncodingKind[] {
  const trimmed = text.trim();
  if (!trimmed) return [];
  const candidates: EncodingKind[] = [];
...
export function normalizeHexInput(text: string): string {
...
export function base64ToBytes(text: string): Uint8Array | null {
  ...
  for (const ch of body) {
    ...
    if (value === -1) return null; // unreachable after BASE64_RE, kept defensive
...
export function base58CheckDecode(text: string): { version: number; payload: Uint8Array } | null {
...
// ---------------------------------------------------------------------------
// Bech32 / bech32m segwit addresses, BIP 173 with the BIP 350 amendment:
// witness v0 keeps the bech32 checksum constant (1), v1+ uses bech32m
// (0x2bc830a3) — the change that fixed bech32's insertion malleability.
// ---------------------------------------------------------------------------
...
export function segwitAddressDecode(text: string): SegwitAddress | null {
  const trimmed = text.trim();
  // BIP 173 case rules: all-lower or all-upper, never mixed; 90 chars max.
  if (trimmed.length === 0 || trimmed.length > 90 || isMixedCase(trimmed)) return null;
  const lowered = trimmed.toLowerCase();
  const separator = lowered.lastIndexOf("1");
  // Nonempty hrp; data part needs at least a version char + 6 checksum chars.
  if (separator < 1 || lowered.length - separator - 1 < 7) return null;
  ...
  if (programBytes === null) return null;
...
export function segwitAddressEncode(hrp: string, version: number, program: Uint8Array): string | null {
  // Canonical output is lowercase, so an uppercase hrp is rejected rather
  // than silently case-folded.
  if (hrp.length === 0 || /[A-Z]/.test(hrp)) return null;
  for (const ch of hrp) {
    const code = ch.charCodeAt(0);
    if (code < 33 || code > 126) return null;
  }
  ...
  let address = `${hrp}1`;
  for (const value of [...data, ...checksum]) address += BECH32_CHARSET[value];
  return address.length <= 90 ? address : null;
}

// ---------------------------------------------------------------------------
// scriptPubKey <-> address. Only the three template script families HAVE an
// address form (p2pkh, p2sh, witness v0..v16 single-push); everything else
// (bare multisig, op_return, ...) is representable only as script hex.
// ---------------------------------------------------------------------------

export type Network = "bitcoin" | "testnet" | "signet" | "regtest";

function hrpForNetwork(network: Network): string {
  switch (network) {
    case "bitcoin":
      return "bc";
    case "testnet":
...
function networksForHrp(hrp: string): Network[] | null {
  switch (hrp) {
    case "bc":
      return ["bitcoin"];
    case "tb":
...
function networksForBase58Version(version: number): Network[] | null {
  switch (version) {
...
export function addressFromScript(scriptHex: string, network: Network): string | null {
...
export function scriptFromAddress(text: string): { scriptHex: string; networks: Network[] } | null {
  const trimmed = text.trim();
  const segwit = segwitAddressDecode(trimmed);
  if (segwit) {
    const networks = networksForHrp(segwit.hrp);
    if (networks === null) return null; // valid bech32, but not a bitcoin network hrp
  ...
  if (check && check.payload.length === 20) {
    ...
    const scriptHex =
      check.version === 0x00 || check.version === 0x6f
        ? `76a914${hash}88ac`
        : `a914${hash}87`;
...
// ---------------------------------------------------------------------------
// parseFlexible: the context filter. Mirrors the FormResult pattern from
// src/session/state.ts (never throws, errors render inline); additionally
// carries WHICH encoding matched (kind), the canonical rendering for the
// context, and an optional note naming any conversion that was applied.
//
// kind values: "hex" | "base64" (byte/psbt contexts), "segwit-address" |
// "base58check-address" | "script" (what the input WAS), "address" (script
// context fed an address).
// ---------------------------------------------------------------------------

export type ParseContext = "hex-bytes" | "hex-bytes-32" | "address" | "script" | "psbt";

// Each branch also carries the OTHER branch's fields as optional
// undefined-typed stamps: the lax emit config (tsconfig.json, no
// strictNullChecks) does not narrow discriminated unions through `!x.ok`, so
// without the stamps callers there could not read `.error`/`.canonical` at
// all. Runtime shape and strict-mode behavior are unchanged.
export type FlexibleParse =
  | { ok: true; kind: string; canonical: string; note?: string; error?: undefined; tried?: undefined }
  | { ok: false; error: string; tried: string[]; kind?: undefined; canonical?: undefined; note?: undefined };

export function parseFlexible(text: string, context: ParseContext, network: Network = "bitcoin"): FlexibleParse {
  const trimmed = text.trim();
  if (!trimmed) return { ok: false, error: "empty input", tried: [] };
  switch (context) {
    case "hex-bytes":
...
function parseBytes(trimmed: string, exactBytes: number | null): FlexibleParse {
...
function parseAddressInput(trimmed: string, network: Network): FlexibleParse {
  ...
  if (segwit) {
    ...
    if (canonical !== null) return { ok: true, kind: "segwit-address", canonical };
  }
  const check = base58CheckDecode(trimmed);
...
function parseScriptInput(trimmed: string): FlexibleParse {
  ...
  return {
    ok: false,
    error:
      "not a scriptPubKey or a convertible address (tried: script hex, segwit address, base58check address)",
    tried,
  };
}
...
function parsePsbtInput(trimmed: string): FlexibleParse {
  ...
  return {
    ok: false,
    error:
      "not a PSBT: neither base64 starting cHNidP8 nor hex starting 70736274ff (tried: base64 psbt, hex psbt)",
    tried,
  };
}
