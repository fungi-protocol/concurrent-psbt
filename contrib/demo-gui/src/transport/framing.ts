...
export function frame(value: Uint8Array): Uint8Array<ArrayBuffer> {
...
export function deframe(
  buf: Uint8Array,
): { record: Uint8Array<ArrayBuffer>; rest: Uint8Array<ArrayBuffer> } | null {
  ...
  if (buf.length < 4 + len) return null; // incomplete; wait for more bytes.
...
export function deframeAll(
  buf: Uint8Array,
): { records: Uint8Array<ArrayBuffer>[]; rest: Uint8Array<ArrayBuffer> } {
  ...
  let rest: Uint8Array<ArrayBuffer> = buf.slice(0);
  for (;;) {
