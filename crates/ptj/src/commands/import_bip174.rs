...
pub(super) fn run(config: ImportBip174Config) -> Result<Psbt> {
    let psbt = io::read_bip174(&config.file)?;
...
fn from_bip174(psbt: bip174::Psbt) -> Result<Psbt> {
    ...
    if inputs.len() != unsigned_tx.input.len() {
        return Err(Error::new("BIP 174 input map count does not match unsigned transaction"));
    }
    if outputs.len() != unsigned_tx.output.len() {
        return Err(Error::new("BIP 174 output map count does not match unsigned transaction"));
    }
