...
pub fn scrub(psbt: &[u8]) -> Result<Vec<u8>, Error> {
    ...
    // First read the global map to detect version and determine input/output counts.
    let mut global_pairs: Vec<Pair> = Vec::new();
    while let Some(pair) = Pair::decode(&mut r)? {
        global_pairs.push(pair);
    }
    let (n_inputs, n_outputs) = get_number_of_inputs_and_outputs(&global_pairs)?;

    // All maps are read into a BTreeMap so the retained pairs are emitted in a
    // deterministic, key-sorted order regardless of their input ordering.
    let mut sanitized_global: BTreeMap<Key, Vec<u8>> = BTreeMap::new();
    for pair in global_pairs {
        if GlobalInsensitive::contains(pair.key.type_value) {
            sanitized_global.entry(pair.key).or_insert(pair.value);
        }
    }

    let mut sanitized_inputs: Vec<BTreeMap<Key, Vec<u8>>> = Vec::new();
    for _ in 0..n_inputs {
        let mut map = BTreeMap::new();
        while let Some(pair) = Pair::decode(&mut r)? {
            if InputInsensitive::contains(pair.key.type_value) {
                map.entry(pair.key).or_insert(pair.value);
            }
        }
        sanitized_inputs.push(map);
    }

    let mut sanitized_outputs: Vec<BTreeMap<Key, Vec<u8>>> = Vec::new();
    ...
    // Write each map to the final buffer, terminating every map with a 0x00 separator.
    let mut out = Vec::with_capacity(psbt.len());
    out.extend_from_slice(&MAGIC);

    for map in std::iter::once(&sanitized_global)
        .chain(sanitized_inputs.iter())
        .chain(sanitized_outputs.iter())
    {
        for (key, value) in map {
            encode_pair(&mut out, key, value);
        }
        out.push(0x00);
    }
...
fn encode_pair(out: &mut Vec<u8>, key: &Key, value: &[u8]) {
    out.extend_from_slice(&serialize(&VarInt::from(key.key.len() + 1)));
    out.extend_from_slice(&serialize(&key.type_value));
    out.extend_from_slice(&key.key);
    out.extend_from_slice(&serialize(&VarInt::from(value.len() as u64)));
...
mod tests {
    ...
    mod unit {
        ...
        fn kv(type_value: u8, key_suffix: &[u8], val: &[u8]) -> Vec<u8> {
            ...
            let mut buf = Vec::new();
            encode_pair(&mut buf, &pair.key, &pair.value);
            buf
        }
        ...
        /// Encode a map's pairs in the key-sorted order `scrub` emits, terminated by 0x00.
        fn sorted_map(pairs: &[Vec<u8>]) -> Vec<u8> {
            let mut decoded: Vec<Pair> = pairs
                .iter()
                .map(|p| Pair::decode(&mut p.as_slice()).unwrap().unwrap())
                .collect();
        ...
        fn v2_global(input_count: u8, output_count: u8, extra: &[Vec<u8>]) -> Vec<u8> {
            let mut pairs = vec![
                kv_global(GlobalInsensitive::Version, &[2, 0, 0, 0]),
                kv_global(GlobalInsensitive::TxVersion, &[2, 0, 0, 0]),
                kv_global(GlobalInsensitive::InputCount, &[input_count]),
                kv_global(GlobalInsensitive::OutputCount, &[output_count]),
            ];
        ...
        fn v2_detected_when_another_field_precedes_version() {
            ...
            let global = sorted_map(&[
                kv_global(GlobalInsensitive::TxVersion, &[1, 0, 0, 0]),
                kv_global(GlobalInsensitive::Version, &[2, 0, 0, 0]),
                kv_global(GlobalInsensitive::InputCount, &[0]),
                kv_global(GlobalInsensitive::OutputCount, &[0]),
            ]);
        ...
        fn v0_unsigned_tx_found_after_another_global() {
            ...
            // Output is key-sorted, so UNSIGNED_TX (0x00) now comes before VERSION (0xFB).
            let mut expected = MAGIC.to_vec();
            expected.extend(sorted_map(&[unsigned_tx, version]));
            expected.push(0x00); // empty input map
            expected.push(0x00); // empty output map

            assert_eq!(scrub(&psbt).unwrap(), expected);
        }
    ...
    mod prop {
        ...
        /// Builds a PSBT carrying `pair`, and returns it alongside the pair that scrubbing must
        /// retain.
        fn build_psbt_with_insensitive(field: InsensitiveField, pair: &[u8]) -> (Vec<u8>, Vec<u8>) {
            ...
            let preamble = match field {
                InsensitiveField::Global(key @ GlobalInsensitive::Version)
                | InsensitiveField::Global(key @ GlobalInsensitive::TxVersion) => {
                    Some(encoded_global(key, vec![2, 0, 0, 0]))
                }
                InsensitiveField::Global(key @ GlobalInsensitive::InputCount)
                | InsensitiveField::Global(key @ GlobalInsensitive::OutputCount) => {
                    Some(encoded_global(key, vec![0]))
                }
                _ => None,
            };
            let is_global = matches!(field, InsensitiveField::Global(_));
            let extra_global: &[&[u8]] = if is_global && preamble.is_none() {
                &[pair]
            } else {
                &[]
            };
            ...
            (test_psbt, preamble.unwrap_or_else(|| pair.to_vec()))
        }
        ...
        proptest! {
            ...
            fn insensitive_fields_preserved(
                ...
                let (test_psbt, expected) = build_psbt_with_insensitive(field, &pair);

                let result = scrub(&test_psbt).unwrap();
                prop_assert!(result.windows(expected.len()).any(|w| w == expected));
            }
